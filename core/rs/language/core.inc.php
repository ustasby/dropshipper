...
class Core
{
    ...
    public static
        ...
        $messages = array(), //Сообщения текущей локали для PHP
        $messages_js = array(), //Сообщения текущей локали для JS
        ...
        if ($cache_enabled) {
                list(self::$messages, 
                     ...
                     self::$messages_js_lastmodify
                                ...
                                ->request(array(__CLASS__, 'init'), false, self::getCurrentLang());
        } else {
    ...
    */
    public static function getSystemLanguages($cache_enabled = false)
    {
        if ($cache_enabled) {
                return CacheManager::obj()
                                ->expire(0)
                                ->request(array(__CLASS__, 'getSystemLanguages'), false);
        } else {
    ...
    /**
    * Подключает нужный языковый файл в директори
    */
    ...
    */
    public static function setSystemLang($lang)
    {
        ...
        if (!$is_admin_zone || in_array(strtolower($lang), array_keys($system_langs))) {
            ...
            \RS\Application\Application::getInstance()->headers->addCookie($cookie_name, $lang, time()+60*60*24*365*2, '/');
            return true;
        }
        return false;
    }
        ...
        if (!isset(self::$cache_current_lang)) {
            ...
            if (\RS\Router\Manager::obj()->isAdminZone()) { 
                ...
                if ($request->cookie(self::COOKIE_CUSTOMER_LANG, TYPE_STRING)) {
                    ...
                    $current_lang = ($site) ? $site->language : $request->cookie(self::COOKIE_CUSTOMER_LANG, TYPE_STRING, self::DEFAULT_LANG);
                }
            }
    ...
    */
    public static function translate($phrase, $params = array(), $alias = null)
    {
        ...
        $phrase = preg_replace_callback('/(\[(.*?):%(.*?):(.*?)\])/', array(__CLASS__, 'execPlugin'), $phrase);        
...
namespace {
    ...
    * @return string
    */
    function t($phrase, $params = array(), $alias = null) {
        return \RS\Language\Core::translate($phrase, $params, $alias);
    }
}
