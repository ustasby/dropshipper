...
class Paginator
{
    ...
    * @return Paginator
    */
    function __construct(&$page, $total, $per_page, $pattern_type_or_routeid = self::PATTERN_KEYREPLACE, $route_params = array(), $pattern = 'p')
    {
        $this->total = $total;
    ...
    function getPageHref($page)
    {
        switch($this->pattern_type) {
            case self::PATTERN_KEYREPLACE: {
                if ($page == 1){
                    $href = \RS\Http\Request::commonInstance()->replaceKey(array($this->pattern => null));
                }else{
                    $href = \RS\Http\Request::commonInstance()->replaceKey(array($this->pattern => $page)); 
                }
                break;
            }
            ...
            default: {
                $href = \RS\Router\Manager::obj()->getUrl($this->pattern_type, array($this->pattern => $page)+$this->route_params);
            }
        }
        return $href;
    }
