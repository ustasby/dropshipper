...
class DbMap
{
    ...
    */
    ...
    {
        $queries = array();
        $fields = array(); //директивы sql полей
    ...
    */
    private function isSyncField($name)
    {
        ...
        $maxlen = ($property->hasLength()) ? "({$property->getMaxLength()})" : null;
        $null = ($dbfield["Null"]=="YES") == $property->isAllowEmpty();
        $type = ($dbfield["Type"]==$property->getSqlNotation() . $property->getSQLTypeParameter());
        $default = ($property->getDefault() === false) || $property->getDefault() === $dbfield['Default'];
