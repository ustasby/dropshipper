    ...
    function getHookHandlers($cache = true)
    {
        if ($cache) {
            return \RS\Cache\Manager::obj()
                ->request(array($this, __FUNCTION__), false, $this->site_id);
        } else {
            $hook_handlers = array();
        ...
        if (is_dir($hook_root)) {
            ...
            foreach (new \DirectoryIterator($hook_root) as $dir_info) {
                ...
                foreach(new \DirectoryIterator($dir_info->getPathname()) as $file_info) {
                    ...
                    $file = pathinfo($tmp_filename) + array('extension' => '');
                    
                    if ( in_array($file['extension'], array('pre', 'post', '')) ) {
    ...
    */
    private function addDebugComments($html, $template)
    {
        if (\RS\Debug\Mode::isEnabled()) {
            ...
            $comment_open = '<!-- '.t("Блок добавлен с помощью шаблона '%0'", array($template)).' -->';
            $comment_close = '<!-- '.t("Конец блока, добавленного с помощью шаблона '%0'", array($template)).' -->';            
