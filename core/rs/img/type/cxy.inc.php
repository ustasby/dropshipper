...
* Запрошенное изображение ищется в центре исходного. 
* Итоговое изображение всегда будет запрошенного размера.
* 
* Crop XY
*/
class Cxy extends AbstractType
{
    public 
    ...
    public function resizeImage(\RS\Img\File $srcImage, $dstImageFileName, $final_width, $final_height, $quality = 90)
    {
        ...
        if ($possible_w <= $w_image) {
            ...
            $offset_y = round($h_image/2 - $calc_h/2);
            $offset_x = 0;
        }
        
        $proxy_image = new \RS\Img\Create($final_width, $final_height, $srcImage->type);
