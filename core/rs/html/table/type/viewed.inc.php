...
class Viewed extends AbstractType
{
    ...
    function __construct($field, $meter_api, $title = null, $property = null)
    {
        ...
        $this->setTdAttr(array(
            'class' => 'cell-is-viewed'
        ));
        $this->setThAttr(array(
            'class' => 'cell-is-viewed'
        ));
        ...
        if ($meter_api instanceof MeterApiInterface) {
            ...
            throw new \RS\Exception(t('Ожидался объект, потомок MeterApiInterface'));
        }
        ...
        $this->setViewOneUrlPattern($router->getAdminPattern('markOneAsViewed', array(':id' => '@'.$this->object_id_field)));
