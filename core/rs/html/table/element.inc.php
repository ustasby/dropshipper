...
class Element extends \RS\Html\AbstractHtml
{
    protected
        $data,
        $table_attr = array('class' => 'rs-table'),
        $row_attr_param,
        $tr_param = array(),
    ...
    */
    function addColumn(Type\AbstractType $column, $pos = null) 
    {
        ...
        if ($pos !== null) {
            $this->columns = array_merge(array_slice($this->columns, 0, $pos), array($column), array_slice($this->columns, $pos));
        } else {
            $this->columns[] = $column;
        }
        return $this;
            ...
            foreach($data as $row => $val)  {
                foreach ($this->columns as $col => $coltype_obj) {
                    if (empty($coltype_obj->property['hidden'])) {
                        ...
                        if (isset($field))
                        {
                            if ($val instanceof \RS\Orm\AbstractObject && isset($val['__'.$field])) {
