...
class Control extends \RS\Html\AbstractHtml
{
    ...
    protected $exclude_get_params = array('p');
    ...
    function __construct(array $options = array())
    {
        ...
        /*
        if ($this->container && $this->auto_fill) {
            $this->fill();
        }
        */
    }
    ...
    */
    function getCleanFilterUrl()
    {
        ...
        return \RS\Http\Request::commonInstance()->replaceKey(array($this->getFilterVar() => null) + $exclude );
    }
    ...
    */
    ...
    {
        if (!isset($_COOKIE[$this->uniq])) return array();
    ...
    * 
    * @return \RS\Html\Filter\Container[]
    */
    function getAllContainers()
    {
        return array_merge(array($this->container), $this->container->getSecContainers());
    }
    ...
    * 
    * @param \RS\Orm\Request $q исходный объект запроса
    * @return \RS\Orm\Request исходный объект запроса с установленными фильтрами
    */
    function modificateQuery(\RS\Orm\Request $q)
    {
        ...
        foreach ($items as $item) {
            /**
             * @var \RS\Html\Filter\Type\AbstractType $item
             */
