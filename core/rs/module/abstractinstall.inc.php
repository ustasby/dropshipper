...
*/
class AbstractInstall implements \RS\Module\InstallInterface
{
    protected
        $module,
        $mod_folder,
    ...
    */    
    function update()
    {
        ...
        foreach($orm_objects as $object) {
            if (!$object->dbUpdate()) {
                $this->addError(t("Не удалось обновить базу данных объекта '%0'", array(get_class($object))));
                return false;
            }   
        }
        
        //Ставим отметку, что модуль установлен
        $self_module = new Item($this);
        $config = $self_module->getConfig();
    ...
    private function installTemplate($src_path)
    {
        ...
        {
            return $this->addError(t("Не найден файл параметров темы '%0'", array($theme_xml)));
        }

        // Перенос файлов темы
        $theme_name = $this->module.'_theme';
        $dest_folder = \Setup::$SM_TEMPLATE_PATH.'/'.$theme_name;
        Tools::deleteFolder($dest_folder);
        rename($src_path, $dest_folder);
    ...
    * @return bool|array
    */
    protected function importCsvFiles($demo_schemas_array=array(), $charset='utf-8', $params = array())
    {
        $params += array(
            'schema' => null,
            'file' => null,
        ...
        $class_found = false;
        $check_timeout = \RS\Config\Loader::byModule('main')->csv_check_timeout;
    ...
    */
    ...
    {        
        ...
        $this->addError( t('Не найден файл с демо данными %0', array($csv_file)) );
        return false;
    }
