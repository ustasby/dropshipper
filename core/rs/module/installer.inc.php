...
class Installer extends \RS\Module\AbstractModel\BaseModel
{
    ...
    protected
        ...
        $allow_mime = array('application/zip'),
        $path,
        $prefix,
        $error = false,
    ...
    */
    function extractFromPost($postVar)
    {
        ...
        if (($error = \RS\AccessControl\Rights::CheckRightError('modcontrol', ACCESS_BIT_WRITE)) !== false ) {
    ...
    */
    function validateTmp()
    {
        ...
        if (isset($this->module_info['core_version']) && !$this->checkCoreVersion($this->module_info['core_version'])) {
            $this->addError( t('Версия ядра не соответствует требованиям модуля (%0)', array($this->module_info['core_version'])) );
        }
    ...
    function uninstallModules($aliases)
    {
        ...
        foreach ($aliases as $modname) {
            ...
            if (($uninstall_result = $module->uninstall()) !== true ) {
                ...
                $this->addError( t("Ошибка при удалении модуля '%module':%error", array('module' => $modname, 'error' => $err)) );
            }
        }
