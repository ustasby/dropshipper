...
class ProductsSerialized extends AbstractPreset
{
    ...
    */
    ...
    function setExportPattern($pattern)
    {
        ...
        $this->import_pattern = str_replace(array('{products}', '{groups}'), array('(?P<products>.*?)', '(?P<groups>.*?)'), quotemeta($pattern));
    }
    ...
    */
    function getColumnsData($n)
    {
        ...
        $export_str = str_replace(array('{groups}', '{products}'), array(implode($this->delimeter, $groups), implode($this->delimeter, $products)), $this->export_pattern);
        return array(
            ...
            if (preg_match("/^{$this->import_pattern}$/", $this->row['products'], $match)) {
                ...
                foreach($groups as $group) {
                    if (!isset(self::$cache_groups[$group])) {
                        $item = \Catalog\Model\Orm\Dir::loadByWhere(array(
                ...
                foreach($products as $product) {
                    if (!isset(self::$cache_products[$product])) {
                        $item = \Catalog\Model\Orm\Product::loadByWhere(array(
