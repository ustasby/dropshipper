...
abstract class AbstractCompress
{
    ...
    */
    ...
    {        
        $uniq_t = array('file' => $group['uniq']['file'].'?t'.$group['time'])  //Добавляем параметр (чтобы браузер не использовал кэшированный файл)
                    + $group['files'][0];
    ...
    */
     ...
     * @param $compress_type тип сжатия: 0 - не сжимать, 1 - объединять, 2 - объединять, сжимать
     * @return array
     */
    public function getCompressed($no_compress = array(), $compress_type)
    {
        ...
        foreach ($this->file_list as $name => $filearr)
        {
            ...
            $params = array_diff_key($filearr['params'], array('unshift' => true, 'endGroup' => true, 'header' => true, 'footer' => true));
            ...
            if (!isset($no_compress[$name]) && file_exists(\Setup::$PATH.$pfilename))
            {
                if (!isset($groups[$i])) {
                    $groups[$i] = array(
                        'files' => array(),
        ...
        foreach ($groups as $group)
        {
            if (!isset($group['uniq'])) {
                $group['uniq'] = 
                    array('file' => $this->output_path.'/'.sprintf("%u", crc32($group['uniq_src'])).$this->extension)
                    + $group['files'][0];
            }
