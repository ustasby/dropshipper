    ...
    */
    static function attachClassBehavior(BehaviorAbstract $behavior_class)
    {
        if (!isset(self::$class_behaviors[get_called_class()])) {
            self::$class_behaviors[get_called_class()] = array();
        }
    ...
    */
    public function behaviorMethodExists($method_name)
    {
        $behaviors = $this->instance_behaviors + 
                        (isset(self::$class_behaviors[get_called_class()]) ? self::$class_behaviors[get_called_class()] : array());
        
        foreach($behaviors as $class => $behavior_template) {
            if (is_callable(array($behavior_template, $method_name))) {
                return true;
            }
        }
        return false;
    ...
    {                    
        ...
        foreach($behaviors as $class => $behavior_template) {
            if (is_callable(array($behavior_template, $method_name))) {
                ...
                return call_user_func_array(array($this->cache_behavior_instances[$class], $method_name), $arguments);
            }
        }
        
        throw new Exception(t('Метод %0 не найден в классе %1', array($method_name, $this->_self_class) ));
    }
}
