...
function smarty_function_modulegetvars($params, &$smarty)
{
    ...
    if (!isset($params[\RS\Controller\Block::BLOCK_ID_PARAM])) {
        if (!isset($block_iterator[$template->source->filepath])) {
            $block_iterator[$template->source->filepath] = 1;
        } else {
            $block_iterator[$template->source->filepath]++;
        }
        //принимаем за block_id - полный путь к шаблону и порядковый номер блока в шаблоне
        $params[\RS\Controller\Block::BLOCK_ID_PARAM] = $template->source->filepath.$block_iterator[$template->source->filepath];
    }
