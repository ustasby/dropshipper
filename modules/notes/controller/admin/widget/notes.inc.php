...
class Notes extends \RS\Controller\Admin\Widget
{
    ...
    {
        $filter_creator = $this->getRequestParam('notes_filter_creator', array(
            self::NO_FILTER,
            self::FILTER_CREATOR_MY));

        $filter_status = $this->getRequestParam('notes_filter_status', array(
            self::NO_FILTER,
            self::FILTER_STATUS_CLOSED,
            self::FILTER_STATUS_UNCLOSED));

        $sort = $this->getRequestParam('notes_sort', array(
            self::SORT_CREATE,
            self::SORT_UPDATE
            ));
        ...
        elseif($filter_status == self::FILTER_STATUS_UNCLOSED) {
            $api->setFilter('status', array(Note::STATUS_OPEN, Note::STATUS_INWORK), 'in');
        }
        ...
        $paginator = new \RS\Helper\Paginator($page,
                                              ...
                                              $page_size,
                                              'main.admin',
                                              array('mod_controller' => $this->getUrlName()));
    ...
    function getTools()
    {
        ...
        return array(
            array(
                ...
                'href' => $router->getAdminUrl('add', array('context' => 'widget'), 'notes-notectrl'),
            ...
            array(
                ...
                'href' => $router->getAdminUrl(false, array(), 'notes-notectrl')
            )
        );
    }
}