...
class Newlist extends \RS\Controller\Admin\Widget
{
    ...
    {
        parent::__construct($param + array('prefix_get' => 'wnl'));
    }
    ...
    function actionIndex()
    {
        ...
        $total = \RS\Cache\Manager::obj()
            ...
            ->request(array($api, 'getListCount'));
        
        $list = \RS\Cache\Manager::obj()
            ->watchTables($api->getElement()) //Кэш до изменения таблиц
            ->request(array($api, 'getList'), $page, $this->pageSize);

        $paginator = new \RS\Helper\Paginator($page, $total, $this->pageSize, 'main.admin', array('mod_controller' => $this->getUrlName()));
