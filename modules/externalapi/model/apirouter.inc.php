...
{    
    ...
    public static function getMethodsVersions($cache = true)
    {
        if ($cache) {
            return \RS\Cache\Manager::obj()
                ->request(array(__CLASS__, __FUNCTION__), false);
        } else {
            $versions = array();
    ...
    */
    public static function getMethodsLanguages($cache = true)
    {
        if ($cache) {
            ...
            $languages = array(\ExternalApi\Model\AbstractMethods\AbstractMethod::DEFAULT_LANGUAGE => 1);
    ...
    */
    public static function getMethodInstance($method)
    {
        $method = strtolower($method);
        $methods_list = self::getApiMethods();
    ...
    * ]
    */
    public static function getApiMethodsSelectList(array $root_item = array(), $only_allowable = false, $lang = null, $cache = true)
    {
        $result = array();
        ...
        if (!isset($scope_cache[$site_id]) || !$cache) {
            ...
            if ($cache) {
                ...
                $scope_cache[$site_id] = \RS\Cache\Manager::obj()
                    ->request(array(__CLASS__, __FUNCTION__), false, $only_allowable, $site_id);
                    
            } else {
                $exists_methods = array();
                ...
                if (!$dir->isDot() && $dir->isDir()) {
                    ...
                    foreach($files_iterator as $filename => $file) {
                        ...
                        if (is_subclass_of($class, '\ExternalApi\Model\AbstractMethods\AbstractMethod')) {
                            ...
                            $exists_methods[strtolower($method_fullname)] = array(
