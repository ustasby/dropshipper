...
class ValidateArray
{
    ...
    */
    function validate($param_name, $param_value, $full_data)
    {
        if (!is_array($param_value)) {
            throw new ApiException(t('Параметр %0 должен быть массивом', array($param_name)), ApiException::ERROR_WRONG_PARAM_VALUE);
        }
    ...
    */
    protected function recursiveValidation($schema, $param_name, $param_value, $full_data, $path = array())
    {
        if ($full_data === null) {
        ...
        foreach($schema as $key => $value) {
            ...
            if (!$path && $key == '@validate_callback') {
                ...
                if ($validate_result === false) {
                    throw new ApiException(t('Недопустимое значение в переменной %1', array($param_name)), ApiException::ERROR_WRONG_PARAM_VALUE);
                }
            ...
            if (is_array($value)) {
                ...
                $result = $this->recursiveValidation($value, $param_name, $next_param_value, $full_data, array_merge($path, array($key)));
                if ($next_param_value !== null) {
                    $param_value[$key] = $result;
                }
                
            } else {
                throw new ApiException(t('Некорректная схема валидации параметра %0. Ожидался массив в ключе %1', array($param_name, $var_path)), ApiException::ERROR_INSIDE);
            }
        }                   
        
        //Проверяем на наличие лишних элементов
        if (is_array($param_value)) {
            $allowable_keys = array();
            ...
            if ($allowable_keys) {
                ...
                if ($extra_keys) {
                    throw new ApiException(t("Обнаружены лишние элементы массива %0 в ключе '%1' в параметре {$param_name}", array(implode(',', $extra_keys), $cur_path)), ApiException::ERROR_WRONG_PARAM_VALUE);
                }
            }
        }
    ...
    */
    protected function validateBranch($key, $schema_orig, $param_name, $param_value, $cur_path, $full_data)
    {
        ...
        if (!empty($schema['@require']) && !isset($param_value[$key])) {
            throw new ApiException(t('Не найден ключ %0 в параметре %1', array($cur_path, $param_name)), ApiException::ERROR_WRONG_PARAM_VALUE);
        }
        ...
        if (isset($param_value[$key])) {
            ...
            if (isset($schema['@allowable_values']) && !in_array($param_value[$key], $schema['@allowable_values'])) {
                throw new ApiException(t('Не допустимое значение в ключе %0 в переменной %1', array($cur_path, $param_name)), ApiException::ERROR_WRONG_PARAM_VALUE);
            }
            ...
            if (isset($schema['@validate_callback'])) {
                ...
                if ($validate_result === false) {
                    throw new ApiException(t('Недопустимое значение в ключе %0 в переменной %1', array($cur_path, $param_name)), ApiException::ERROR_WRONG_PARAM_VALUE);
                }
