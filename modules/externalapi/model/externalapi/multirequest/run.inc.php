    ...
    protected function process($requests)
    {
        ...
        foreach($requests as $id => $request) {
            ...
            if (!isset($request['url']) && !isset($request['method'])) {
                throw new ApiException(t('В запросе %0 должен присутствовать ключ url или method', array($id)), ApiException::ERROR_WRONG_PARAM_VALUE);
            }
            ...
            try {
                ...
                if (strtolower($method) == strtolower($this->getSelfMethodName())) {
                    throw new ApiException(t('Метод %0 не может быть вызван', array($method)), ApiException::ERROR_WRONG_PARAM_VALUE);
                }
