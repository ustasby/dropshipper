...
abstract class AbstractMethod
{
    ...
    */
    public function run($params, $version = null, $lang = 'ru')
    {
        ...
        $event_result = \RS\Event\Manager::fire('api.'.strtolower($this->getSelfMethodName()).'.before', array(
            'result' => array(),
        ...
        $response = array_merge_recursive($response_before, call_user_func_array(array($this, $this->getProcessFunctionName($this->version)), $this->method_params));

        //Разрешаем корректировать результат выполнения метода другим модулям
        $event_result = \RS\Event\Manager::fire('api.'.strtolower($this->getSelfMethodName()).'.success', array(
            'result' => $response,
            'version' => $this->version,
            'lang' => $this->lang,
            'method' => $this,
    ...
    */
    public function getProcessFunctionName($version)
    {
        ...
        if (!in_array($version, $versions)) {
            throw new ApiException(t("API указанной версии '%0' не существует. Допустимы версии: %1", array($version, implode(',', $versions))), ApiException::ERROR_WRONG_PARAMS);
        }
        ...
        if (is_callable(array($this, $default_func_name))) {
            return $default_func_name;
        } else {
            throw new ApiException(t('Отсутствует метод process в классе %0', array(get_class($this))), ApiException::ERROR_INSIDE);
        }
    }
    ...
    public function validateParams($params, $version)
    {
        ...
        foreach($method_allow_params as $name => $info) {
            if (!$info['is_optional'] && !isset($params[$name])) {
                $error = new ApiException(t('Передан неверный набор параметров. Не найден параметр %0', array($name)), ApiException::ERROR_WRONG_PARAMS);
        ...
        if (count($params)) {
            $error = new ApiException(t("Передан неверный набор параметров. Обнаружены неизвестные параметры '%0'", array(implode(', ', array_keys($params)))), ApiException::ERROR_WRONG_PARAMS);
            $this->addMethodHelpUrlToException($error);            
        ...
        if (\RS\Config\Loader::byModule($this)->enable_api_help) {
            $api_exception->addExtraApiData('help_url', \RS\Router\Manager::obj()
                                                   ->getUrl('externalapi-front-apigate-help', 
                                                            array('method' => $this->getSelfMethodName()), true));
        }
        ...
        foreach($methods as $method) {
            if (preg_match('/^process(Ver(.*?))?$/', $method->name, $match)) {
                ...
                foreach ($method->getParameters() as $param) {
                    ...
                    $params['params'][$clean_name] = array(
                        'name' => $clean_name,
