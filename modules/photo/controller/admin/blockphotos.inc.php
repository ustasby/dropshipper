...
class BlockPhotos extends \RS\Controller\Admin\Block
{
    ...
    function actionIndex()
    {
        $this->view->assign(array(
            'addParam' => array('linkid' => $this->param['linkid'], 'type' => $this->param['type']), 
    ...
    */
    function actionAddphoto()
    {
        ...
        $items = array();
        $files = $this->url->files('files', TYPE_ARRAY, array());        
        if (!empty($files))
        {
            $photo_api = new \Photo\Model\PhotoApi();
            $normalize_files = array();
            ...
            foreach ($normalize_files as $key => $file)
            {
                if ($file['error'] != UPLOAD_ERR_NO_FILE) {
                    ...
                    if ($photo !== false) {
                        $this->view->assign('photo_list', array($photo));
    ...
    */
    ...
    {
        if ($this->noWriteRights()) return $this->result->getOutput();
