                                   ...
                                   $id_field = 'id')
    {
        if (is_object($total_or_object)) {
            ...
            if ($total_or_object instanceof AbstractObject) {
                ...
                if ($site_id_field) {
                    $total_or_object->where(array($site_id_field => $this->site_id));
                }
            }

            if ($total_or_object instanceof Request) {
                $total_or_object->where("{$id_field} > '#id'", array(
                                    ...
                                    $id_field = 'id')
    {
        if (is_object($last_id_or_object)) {
            ...
            if ($last_id_or_object instanceof AbstractObject) {
                ...
                if ($site_id_field) {
                    $last_id_or_object->where(array($site_id_field => $this->site_id));
                };
                if ($user_id_field) {
                    $last_id_or_object->where(array($user_id_field => $this->user_id));
                };
            }
