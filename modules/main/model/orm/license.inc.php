...
class License extends \RS\Orm\AbstractObject
{
    ...
    function _init()
    {
        $this->getPropertyIterator()->append(array(
            'license' => new Type\Varchar(array(
                'maxLength' => 24,
                'attr' => array(array(
                    'size' => 50
                )),
                ...
                'company_name' => new Type\Varchar(array(
            ...
            'crypt_type' => new Type\Varchar(array(
                'visible' => false,
    ...
    function beforeWrite($flag)
    {
        if ($flag != self::UPDATE_FLAG) {
            ...
            $crypt_type = function_exists('mcrypt_module_open') ? 'mcrypt' : 'openssl';
    ...
    */
    ...
    function refresh()
    {
        ...
        $type = null;
        $expire = null;
        $data = __GET_LICENSE_DATA($this['license'], $error_msg, array(), $type, $expire, $this['crypt_type']);
        if ($data) {
            $this['data'] = $data;
            return $this->update();
        } else {
            return $this->addError($error_msg);
        }
    }
}