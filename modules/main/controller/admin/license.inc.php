    ...
    function helperIndex()
    {
        $helper = parent::helperIndex()
            ...
            'Columns' => array(
                ...
                new TableType\Usertpl('object', t('Объект лицензирования'), '%main%/license_col_object.tpl', array('TdAttr' => array('class' => 'cell-small'))),
                new TableType\Datetime('date_of_activation', t('Дата активации'), array('format' => 'd.m.Y')),
                new TableType\Text('domain', t('Домен')),
                new TableType\Actions('license_key', array(
                    new TableType\Action\Action($this->router->getAdminPattern('refreshLicense', array(':key' => '~field~')), t('Обновить'), array(
                        'class' => 'crud-get',
                        'iconClass' => 'refresh'
                    ))
                ))
    ...
    function actionAddLic($is_activate = false)
    {
        ...
        if ($this->url->isPost()) 
        {
            ...
            if ($this->api->getElement()->isNeedActivation()) {
                $activation_url = $this->router->getAdminUrl('activateLicense', array(
    ...
    */
    ...
    function actionLicenseUpdate()
    {
        ...
        if ($license->load($key)) {
            ...
            return $this->result->setTemplate('license_tmp_update.tpl');
