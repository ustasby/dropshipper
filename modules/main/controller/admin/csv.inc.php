...
use \RS\Html\Toolbar, 
    \Main\Model\Orm\CsvMap,
    \RS\Html\Toolbar\Button as ToolbarButton;

class Csv extends \RS\Controller\Admin\Front
{ 
    /**
    ...
    */
    function actionExportCsv()
    {
        ...
        $helper = new \RS\Controller\Admin\Helper\CrudCollection($this, null, null, array(
            'topTitle' => t('Выберите колонки для экспорта'),
            'bottomToolbar' => new Toolbar\Element(array(
                'Items' => array(
        ...
        if ($csv_schema = \RS\Csv\AbstractSchema::getByShortName($schema)) {
            ...
            if ($this->url->isPost()) {
                ...
                if (empty($columns)) {
                    return $this->result
                                    ...
                                    ->setErrors(array(
                                        'destination' => array(
                                            ...
                                            'errors' => array(t('Не задана ни одна колонка для экспорта'))
                                        )
                                    ));
                } else {
                    ...
                    return $this->result
                                    ->setSuccess(true)
                                    ->setAjaxWindowRedirect($this->router->getAdminUrl('doexport'));
                }
            }else{
    ...
    */
    function actionDoExport()
    {
        ...
        if (isset($_SESSION['export_data'])) {
            $csv_schema = \RS\Csv\AbstractSchema::getByShortName($_SESSION['export_data']['schema']);
    ...
    */
    function actionSaveMap()
    {
        ...
        $columns = $this->url->request('columns', TYPE_ARRAY);
        $schema = $this->url->request('schema', TYPE_STRING);
        $type = $this->url->convert( $this->url->request('type', TYPE_STRING), array('export', 'import'));
        
        $map = new \Main\Model\Orm\CsvMap();
        $map['schema'] = $schema;
        $map['title'] = $title;
        $map['type'] = $type;
        $map['columns'] = $columns;
    ...
    */
    function actionDeleteMap()
    {
        ...
        $map = new \Main\Model\Orm\CsvMap($id);
        if ($map['id']) {
            $map->delete();
        }
