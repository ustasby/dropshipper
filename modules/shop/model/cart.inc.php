...

{
    ...
    */
    function getCartData($format = true, $use_currency = true)
    {
        ...
        $min_weight_limit = \RS\Config\Loader::byModule($this)->basketminweightlimit;
        $weight_unit = \RS\Config\Loader::byModule('catalog')->getShortWeightUnit();
    ...
    */
    function addDeliveryData($result, $use_currency)
    {
        if ($this->mode != self::MODE_SESSION && $this->order['delivery']>0) {
            ...
            $taxes = \Shop\Model\TaxApi::getDeliveryTaxes($delivery, $this->order->getUser(), $address);
    ...
    */
    protected function addDiscountData($result, $use_currency)
    {
        ...
        foreach($coupon_items as $key => $item) {
            /**
            * @var \Shop\Model\Orm\Discount $coupon
            */
            $coupon = $item['coupon'];   
        ...
        foreach($in_basket as $n => $item) {
            ...
            if (isset($extra['personal_discount'])) {
                ...
                if (strpos($personal_discount, '%') !== false) {
                    $discount = $result['items'][$n]['base_cost'] * (((float)$personal_discount) / 100);
                } else {
                ...
                if ($discount > $result['items'][$n]['discount'] || $result['items'][$n]['discount'] == 0) {
