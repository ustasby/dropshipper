...
class DeliveryPeriod
{
    ...
    public function getPeriodAsText()
    {
        ...
        elseif ($min == $max) {
            return t('%0 [plural:%0:день|дня|дней]', array($min));
        }
        elseif ($min && !$max) {
            return t('от %0 [plural:%0:дня|дней|дней]', array($min));
        }
        elseif (!$min && $max) {
            return t('до %0 [plural:%0:дня|дней|дней]', array($max));
        }
        elseif ($min && $max) {
            return t('от %0 до %1 [plural:%1:дня|дней|дней]', array($min, $max));
        }
