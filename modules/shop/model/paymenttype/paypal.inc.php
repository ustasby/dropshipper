...
class PayPal extends AbstractType
{
    ...
    */
    function getFormObject()
    {
        $properties = new \RS\Orm\PropertyIterator(array(
            ...
            'bussiness_email' => new Type\Varchar(array(
                'description' => t('E-mail продавца на PayPal')
            )),
            'culture' => new Type\Varchar(array(
    ...
    */
    function getPayUrl(Transaction $transaction)
    {
        ...
        $in_curr = $order->currency ? $order->currency : \Catalog\Model\CurrencyApi::getDefaultCurrency()->title;    
        ...
        if ($order_id) {
            $params['item_name'] = t("Заказ №%0. %1", array($order_id, $current_site)); //Id заказа
        } else {
            $params['item_name'] = t("Пополнение баланса лицевого счета. %0", array($current_site)); //Id заказа
        }
    ...
    */
    ...
    function onResult(\Shop\Model\Orm\Transaction $transaction, \RS\Http\Request $request)
    {
        ...
        if($request->request('mc_gross', TYPE_STRING) != $transaction->cost){
            $exception = new ResultException(t('Неверная сумма платежа %0', array($request->request('mc_gross', TYPE_STRING))));
