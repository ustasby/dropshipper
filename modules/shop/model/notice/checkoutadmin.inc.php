                               ...
                               \Alerts\Model\Types\InterfaceDesktopApp
{
    ...
    */
    public function getNoticeDataDesktopApp()
    {
        ...
        $desktop_data->title = t('Оформлен заказ N%0', array($this->order->order_num));
        $desktop_data->short_message = t("%user %nlСумма заказа: %total", array(
            'nl' => "\n",
            'user' => $this->order->getUser()->getFio(),
            'total' => \RS\Helper\CustomView::cost($this->order->totalcost, $this->order->currency_stitle)
        ));
        
        $desktop_data->link = \RS\Router\Manager::obj()->getAdminUrl('edit', array('id' => $this->order->id), 'shop-orderctrl', true);
