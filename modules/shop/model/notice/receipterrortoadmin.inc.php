                               ...
                               \Alerts\Model\Types\InterfaceDesktopApp
{
    ...
    function getNoticeDataEmail()
    {
        ...
        $email_to_admin->subject    = t('Ошибка в чеке кассы по транзакции N%0 на сайте %1', array($this->transaction['id'], \RS\Http\Request::commonInstance()->getDomainStr()));
    ...
    */
    public function getNoticeDataDesktopApp()
    {
        ...
        $desktop_data->title = t('Ошибка в чеке кассы по транзакции N%0', array($this->transaction['id']));
        $desktop_data->short_message = t("Ошибка в чеке кассы по транзакции N%0", array(
            $this->transaction['id']
        ));
        
        $desktop_data->link = \RS\Router\Manager::obj()->getAdminUrl(false, array('transaction_id' => $this->transaction['id']), 'shop-receiptsctrl', true);
