...
/**
* Возвращает список с товарами и элементами в корзине после обновления
*/
class Update extends \ExternalApi\Model\AbstractMethods\AbstractAuthorizedMethod
{
    ...
    */
    ...
    {
        return array(
            self::RIGHT_LOAD => t('Обновление элементов в корзине')
        );
    }
    
    /**
    * Обновляет сведения в корзине и возвращает корзину пользователя со всеми сведениями
    * 
    * @param string $token Авторизационный токен
    * @param array $products массив товаров со сведениями
    * @param string $coupon строка с купоном
    * 
    * @example POST /api/methods/cart.update?products[xzrbcvmpyz][multioffers][123]=Желтый&products[xzrbcvmpyz][offer]=2&products[xzrbcvmpyz][amount]=2
    * 
    * /api/methods/cart.update?products[xzrbcvmpyz][offer]=2&products[xzrbcvmpyz][amount]=2
    * 
    * /api/methods/cart.update?products[xzrbcvmpyz][offer]=2&products[xzrbcvmpyz][amount]=2&coupon=chip
    * 
    * /api/methods/cart.update?products[xzrbcvmpyz][offer]=2&products[xzrbcvmpyz][amount]=2&products[xzrbcvmpyz][concomitant]=34
    * 
    * Ответ:
    * <pre>
    * {
    ...
    */
    protected function process($token = null, $products, $coupon = null)
    {
        ...
        if ($apply_coupon !== true) {
            \Shop\Model\Cart::currentCart()->addUserError($apply_coupon, false, 'coupon');
        }
