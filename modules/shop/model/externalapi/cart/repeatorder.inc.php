...
/**
* Повторяет заказ пользователя и добавляет в корзину товары
*/
class RepeatOrder extends \ExternalApi\Model\AbstractMethods\AbstractAuthorizedMethod
{
    const
        RIGHT_LOAD = 1;
    ...
    */
    ...
    {
        return array(
            self::RIGHT_LOAD => t('Добавление того же заказа в корзину')
        );
    }
    ...
    /**
    * Добавляет товары в корзину пользователя из уже сделанного из заказа
    * 
    * @param string $token Авторизационный токен
    * @param string $order_num Номер заказа
    * 
    * @example GET /api/methods/cart.repeatOrder?token=DsdfvFGkdlvsdfvg5dsfdsvaav34g48dj3r4Rsdfa34&order_num=1
    * Ответ:
    * <pre>
    * {
    *        "response": {
    ...
    */
    protected function process($token, $order_num)
    {
        //Повторим заказ
        $cart = \Shop\Model\Cart::currentCart();
        $cart->repeatCartFromOrder($order_num);
        //Получим корзину
