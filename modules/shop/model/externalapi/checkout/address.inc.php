...
class Address extends \ExternalApi\Model\AbstractMethods\AbstractAuthorizedMethod
{
    ...
    */
    private function checkNoRegiter($user)
    {
        ...
        if ($this->shop_config->require_phone_in_noregister && empty($this->order['user_phone'])){
    ...
    */
    private function checkUserAndAdressFields($type, $user, $address, $regfields_arr, $orderfields_arr)
    {
        ...
        $my_post_vars = array('user_type' => $this->order['user_type'])+ array('only_pickup_points' => $this->order['only_pickup_points'])+ array('use_addr' => $this->order['use_addr']) + $user + $address;
    ...
    */
    private function getAddressValidator()
    {
        if ($this->address_validator === null){
            $this->address_validator = new \ExternalApi\Model\Validator\ValidateArray(array(
    ...
    */
    private function getUserRegisterValidator()
    {
        if ($this->register_validator === null){
            $this->register_validator = new \ExternalApi\Model\Validator\ValidateArray(array(
    ...
    */
    ...
    {   
        ...
        $this->checkUserAndAdressFields($type, $user, $address, $regfields_arr, $orderfields_arr);
        
        //Если регистрация прошла успешно, передадим сведения о пользователе
        if ($type == 'register' && !$this->order->hasError()){ 
            $response['response']['auth'] = array(
