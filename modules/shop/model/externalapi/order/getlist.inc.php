...
class GetList extends \ExternalApi\Model\AbstractMethods\AbstractGetList
{
    ...
    */
    ...
    {
        return array('dateof', 'dateof desc', 'id', 'id desc');
    }
    ...
    public function setFilter($dao, $filter)
    {
        ...
        if ($this->method_params['fulltext_filter']) {
            ...
            if (!$q->issetTable(new \Shop\Model\Orm\OrderItem())) {
                ...
                $q->join(new \Users\Model\Orm\User(), 'A.user_id=U.id', 'U');
            }
            $q->where("(order_num = '#term' OR PRODUCT.title like '%#term%' OR U.name like '%#term%' OR user_fio like '%#term%'  OR U.surname like '%#term%' OR U.e_mail like '%#term%')", array(
    ...
    */
    public function getResultList($dao, $page, $pageSize)
    {
        ...
        $dao->getElement()->getPropertyIterator()->append(array(
            ...
            'dateof_date' => new \RS\Orm\Type\Mixed(array(
                'description' => t('Дата создания в dd.mm.YYYY'),
                'appVisible' => true
            )),
            'dateof_datetime' => new \RS\Orm\Type\Mixed(array(
                'description' => t('Дата создания в dd.mm.YYYY HH:ii'),
                'appVisible' => true
            )),
            'can_online_pay' => new \RS\Orm\Type\Integer(array(
                'description' => t('Возможна ли оплата online'),
                'appVisible' => true
            )),
            'total' => new \RS\Orm\Type\Varchar(array(
    ...
    */
    protected function process($token, 
                               ...
                               $page = "1", 
                               $pageSize = "20", 
                               $sections = array('user', 'status', 'address'))
    {
