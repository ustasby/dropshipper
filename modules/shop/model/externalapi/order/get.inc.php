...
{   
    ...
    */
    ...
    {
        return parent::getRightTitles() + array(
    ...
    */
    private function addOrderItems(\Shop\Model\Orm\Order $order)
    {
        $order->getPropertyIterator()->append(array(
            'dateof_iso' => new \RS\Orm\Type\Mixed(array(
                'description' => t('Дата создания в формате ISO 8601'),
                'appVisible' => true
            )),        
            'dateof_timestamp' => new \RS\Orm\Type\Integer(array(
                'description' => t('Дата создания в формате TIMESTAMP'),
                'appVisible' => true
            )), 
            'dateof_date' => new \RS\Orm\Type\Varchar(array(
                'description' => t('Дата создания в формате dd.mm.YYYYY'),
                'appVisible' => true
            )), 
            'dateof_datetime' => new \RS\Orm\Type\Varchar(array(
                'description' => t('Дата создания в формате dd.mm.YYYYY HH:ii'),
                'appVisible' => true
            )), 
            'items' => new \RS\Orm\Type\Mixed(array(
                'description' => t('Состав заказа'),
                'appVisible' => true
            )),
            'track_url' => new \RS\Orm\Type\Varchar(array(
                'description' => t('Url для отслеживания'),
                'appVisible' => true
            )), 
            'files' => new \RS\Orm\Type\Mixed(array(
                'description' => t('Файлы прикреплённые к заказу'),
                'appVisible' => true
            )),  
            'additional_fields' => new \RS\Orm\Type\Mixed(array(
        ...
        $prototype->getPropertyIterator()->append(array(
            'image' => new \RS\Orm\Type\Mixed(array(
                'description' => t('Фото'),
                'appVisible' => true
            )),
            'url' => new \RS\Orm\Type\Varchar(array(
                'description' => t('Фото'),
                'appVisible' => true
            ))
        ));        
            ...
            if ($payment_id = $object['payment']) {
                ...
                if ($payment->hasDocs()){ //Если есть документы для оплаты
                    $payment->getPropertyIterator()->append(array(
                        'docs' => new \RS\Orm\Type\ArrayList(array(
