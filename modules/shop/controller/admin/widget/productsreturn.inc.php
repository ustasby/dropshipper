...
class ProductsReturn extends \RS\Controller\Admin\Widget
{
    ...
    function actionIndex()
    {
        ...
        $filter = $this->url->convert( $this->myRequest('filter', TYPE_STRING, $default_filter), array(Model\Orm\ProductsReturn::STATUS_NEW, 'all'));
        ...
        if ($filter != 'all') {
            $return_api->setFilter('status',
               array(Model\Orm\ProductsReturn::STATUS_COMPLETE, Model\Orm\ProductsReturn::STATUS_REFUSE) ,
                'notin');
        }
        
        $paginator = new \RS\Helper\Paginator($page, $return_api->getListCount(), $pageSize, 'main.admin', array('mod_controller' => $this->getUrlName()));
