...
class MyProductsReturn extends \RS\Controller\AuthorizedFront
{
    ...
    /**
     * Скно со сгенерированным PDF документом
     *
     * @return string
     */
    function actionPrint()
    {
         ...
         */
        $return = $this->api->getById($return_num, 'return_num');
    ...
    function actionAdd($return_id = null)
    {
        ...
        if (!$return_id){ //Если нужно создать новый возврат
            ...
            if ($order_id) {
                $order    = $this->order_api->getById($order_id, 'order_num');
        ...
        if ($order['user_id'] == $this->user['id']) {
            ...
            if ($this->isMyPost() && $this->url->checkCsrf() && $return->save($return['id'])) {// если пришел POST
                $this->redirect($this->router->getUrl('shop-front-myproductsreturn'));
            }
