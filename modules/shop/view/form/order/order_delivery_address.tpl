{hook name="shop-form-order-order_delivery_address:all" title="{t}Редактирование заказа - блок адреса в диалоге доставки:весь блок{/t}"}    
    <tr>
        <td class="caption">{t}Страна:{/t}</td>
        <td>
            <select id="deliveryCountryIdSelect" name="address[country_id]" data-url="{adminUrl do=getCountryRegions}">
        ...
        <td class="clearfix">
            <div class="inline f">
                    <span {if count($region_list) == 0}style="display:none"{/if} id="region-select">
                        <select id="deliveryRegionIdSelect" name="address[region_id]">
                ...
                <div class="inline">
                    <input id="deliveryCityInput" type="text" value="{$address.city}" name="address[city]">
                    <div class="fieldhelp">{t}Город{/t}</div>
                </div>
        </td>
    </tr>                    
    <tr>
        <td class="caption">{t}Индекс, Адрес:{/t}</td>
        <td class="clearfix">
            <div class="inline f">
                <input id="deliveryZipcodeInput" size="10" type="text" maxlength="20" value="{$address.zipcode}" name="address[zipcode]">
                <div class="fieldhelp">{t}Индекс{/t}</div>
            </div>
            <div class="inline">
                <input size="44" type="text" maxlength="255" value="{$address.address}" name="address[address]">
