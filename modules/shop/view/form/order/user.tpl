<div id="userBlockWrapperContent">
    ...
    {else}
        ...
        {if $order->id > 0}
            {$order_id = $order->id}
        {/if}
        <button id="createUserFromNoRegister" data-href="{$router->getUrl('shop-admin-orderctrl', ['do' => 'createUserFromNoRegister', 'order' => $order_id])}" class="btn btn-default">{t}Создать пользователя из текущего{/t}</button>
    {/if}
</div>