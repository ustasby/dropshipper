...
<p>{t order_num=$order.order_num date={$order.dateof|date_format:"%d.%m.%Y"}}Заказ N%order_num от %date был изменен.{/t}</p>
...
<p>{t}С наилучшими пожеланиями, <br>администрация интернет магазина{/t} {$url->getDomainStr()} {$url->getDomainStr()}.</p>