...
class FileApi extends \RS\Module\AbstractModel\EntityList
{
    ...
    {
        parent::__construct(new Orm\File, array(
            'sortField' => 'sortn',
            'defaultOrder' => 'sortn',            
            'multisite' => true,
        ));
    ...
    */
    function uploadFromUrl($file_path, FilesType\AbstractType $type_class, $link_id, $display_name = null, $upload_type = self::UPLOAD_TYPE_COPY, $info = array())
    {
    ...
    */
    function canUploadFile($file_path, FilesType\AbstractType $type_class, $display_name = null )
    {
        ...
        if ($allowed_extensions && !in_array($ext, $allowed_extensions)) {
            return $this->addError(t(' Недопустимое расширение файла. Допускается:%0', array(implode(', ', $allowed_extensions))));
        }        
    ...
    */
    public static function getTypeClassInstance($type_class_short_name)
    {
        ...
        if (class_exists($type_class_name)) {
            ...
            throw new FilesType\Exception(t('Не найден класс типа связи %0', array($type_class_name)));
        }
    }
