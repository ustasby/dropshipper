...
class FileManager extends \RS\Controller\Admin\Front
{
            ...
            'Items' => array(
                new ToolbarButton\Dropdown(array(
                    ...
                    array(
                        ...
                        'attr' => array(
                            'class' => 'crud-add',
                            'href' => $this->router->getAdminUrl('add', array('path' => $list['epath']['public_dir'], 'ext' => 'tpl'))
                        )
                    ),
                    array(
                        'title' => t('JS - скрипт'),
                        'attr' => array(
                            'class' => 'crud-add',
                            'href' => $this->router->getAdminUrl('add', array('path' => $list['epath']['public_dir'], 'ext' => 'js'))
                        )
                    ),
                    array(
                        'title' => t('CSS - стиль'),
                        'attr' => array(
                            'class' => 'crud-add',
                            'href' => $this->router->getAdminUrl('add', array('path' => $list['epath']['public_dir'], 'ext' => 'css'))
                        )
                    )
                )),
                new ToolbarButton\Space(),
                new ToolbarButton\Button('', t('Создать папку'), array('attr' => array(
                    'class' => 'btn-default makedir',
                    'data-url' => $this->router->getAdminUrl('makeDir', array('path' => $list['epath']['public_dir']))
                ))),
                new ToolbarButton\Button($this->router->getAdminUrl('uploadForm', array('path' => $list['epath']['public_dir'])), t('Загрузить файлы'), array('attr' => array('class' => 'btn-default crud-add'))),
                new ToolbarButton\Button($this->router->getAdminUrl('cloneTheme'), t('Клонировать тему'), array('attr' => array('class' => 'btn-default crud-add')))
            )
        )));
    ...
    function actionAdd($filename = null)
    {
        ...
        if ($filename === null) {
            ...
            $data['overwrite'] = true;
            $helper->setTopTitle(t('Редактировать файл {filename}'), array('filename' => $filename));
        }
    ...
    function actionUploadForm()
    {
        ...
        $helper
            ...
            ->setBottomToolbar(new Toolbar\Element( array(
                'Items' => array(
                    'save' => new ToolbarButton\SaveForm(null, t('Загрузить'), array('noajax' => true, 'attr' => array('class' => 'btn-success start-upload'))),
                    'cancel' => new ToolbarButton\Cancel($this->url->getSavedUrl($this->controller_name.'index')),
                )
            )));
