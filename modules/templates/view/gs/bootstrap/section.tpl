...
{foreach from=$item item=level name="sections"}
    <div class="area {if $level.section.element_type == 'row'}row{else}{*
        ...
        *} {$level.section->getAnyVisibleClass()}" data-section-id="{$level.section.id}">
        ...
        <div class="workarea{if !empty($level.childs) || !$level.section->getBlocks()} sort-sections{/if}{if empty($level.childs) && $level.section.element_type != 'row'} sort-blocks{/if}">
