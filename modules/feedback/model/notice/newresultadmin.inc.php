                     ...
                     implements \Alerts\Model\Types\InterfaceDesktopApp
{
    ...
    */
    public function getNoticeDataDesktopApp()
    {
        ...
        $notice_data->title = t("Получено сообщение №%0 из формы на сайте", array(
            $this->result_item->id
        ));
        $notice_data->short_message = t("Название формы: '%0'", array($this->result_item->getFormObject()->title));
        
        $notice_data->link = \RS\Router\Manager::obj()->getAdminUrl('edit', array('id' => $this->result_item['id']), 'feedback-resultctrl', true);
