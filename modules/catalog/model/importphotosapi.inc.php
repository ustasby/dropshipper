...
class ImportPhotosApi extends \RS\Module\AbstractModel\BaseModel
{
    ...
    */
    function getCompareProductFields()
    {
        ...
        $fields = array('barcode', 'title', 'alias', 'xml_id', 'id', 'maindir', 'brand_id');
        $list = array();
    ...
    */
    function uploadFile($file_arr)
    {
        ...
        $uploader = new \RS\File\Uploader(array('zip'), $this->zip_folder_rel);
        ...
        if ($uploader->setUploadFilename($this->zip_name)->uploadFile($file_arr)) {
            ...
            $error = t('Не удается открыть архив. Ошибка: %0', array($zip_status));
        } else {
    ...
    function extractFile($start_pos = 0)
    {
        ...
        if ($zip->open($zip_filename) === true) {
            ...
            for($i = $start_pos; $i < $zip->numFiles; $i++) {
                ...
                if (substr($file,-1) == '/' || in_array(strtolower(substr($file, -3)), array_merge($this->allow_ext, array('csv')) )) {
                    if (!$zip->extractTo($this->extract_folder, array($file))) {
    ...
    */
    function importPhoto($start_pos = 0, $field, $separator)
    {
        ...
        if ($start_pos == 0) {
            ...
            $this->setParam('touch_products', array());
            $this->cleanLog();
            $this->writeLog('Import start...');
        }
        
        $dir = $this->extract_folder;
        $current_pos = $this->importRecursive($dir, 0, $start_pos, array('field' => $field, 'separator' => $separator), microtime(true));
        ...
        $this->setParam('statistic', array('touch_products' => count($this->getParam('touch_products'))) + $this->getParam('statistic'));
        return $result;
    }
    ...
    */
    private function searchProducts($term, $file, $field)
    {
        $q = \RS\Orm\Request::make()
                ...
                ->where(array('P.site_id' => \RS\Site\Manager::getSiteId()));
                
        switch($field) {
            case 'maindir': $q->join(new Orm\Dir(), 'D.id = P.maindir', 'D')->where(array('D.name' => $term)); break;
            case 'brand_id': $q->join(new Orm\Brand(), 'B.id = P.brand_id', 'B')->where(array('B.title' => $term)); break;
            default: $q->where(array($field => $term));
        }
        ...
        if ($found) {
            ...
            $exclude = \RS\Orm\Request::make()
                ...
                ->where(array('filename' => $file, 'type' => 'catalog'))
