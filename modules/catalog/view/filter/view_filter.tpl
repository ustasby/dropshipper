...
{if $prop->isListType()}
    ...
    <ul class="pr-list">
        {foreach $prop->valuesArr() as $key => $val}
            <li><input id="f_{$prop.id}_{$val@iteration}" type="checkbox" value="{$key}" name="{$name}[{$prop.id}][]" {if is_array($value[$prop.id]) && in_array($key, $value[$prop.id])}checked{/if} class="pr-list">
