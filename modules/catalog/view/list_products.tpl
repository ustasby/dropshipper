...
<div id="products" {if $shop_config}class="shopVersion"{/if}>
    ...
    {hook name="catalog-list_products:options" title="{t}Просмотр категории продукции:параметры отображения{/t}"}
        <div class="viewOptions">
            <a data-href="{urlmake viewAs=table}" class="viewAs table{if $view_as == 'table'} act{/if}" rel="nofollow"></a>
            <a data-href="{urlmake viewAs=blocks}" class="viewAs blocks{if $view_as == 'blocks'} act{/if}" rel="nofollow"></a>
            {t}Сортировать по{/t}:&nbsp;&nbsp;                  
            <div class="lineListBlock sortBlock">
                <a class="lineTrigger rs-parent-switcher">{if $cur_sort=='sortn'}{t}умолчанию{/t}
                                                          ...
                                                          {else}{t}цене{/t}{/if}</a>
                <ul class="lineList">
                    <li><a href="{urlmake sort="sortn" nsort=$sort.sortn}" class="item{if $cur_sort=='sortn'} {$cur_n}{/if}" rel="nofollow"><i>{t}умолчанию{/t}</i></a></li>
                    <li><a href="{urlmake sort="cost" nsort=$sort.cost}" class="item{if $cur_sort=='cost'} {$cur_n}{/if}" rel="nofollow"><i>{t}цене{/t}</i></a></li>
                    <li><a href="{urlmake sort="rating" nsort=$sort.rating}" class="item{if $cur_sort=='rating'} {$cur_n}{/if}" rel="nofollow"><i>{t}популярности{/t}</i></a></li>
                    <li><a href="{urlmake sort="dateof" nsort=$sort.dateof}" class="item{if $cur_sort=='dateof'} {$cur_n}{/if}" rel="nofollow"><i>{t}дате{/t}</i></a></li>
                    <li><a href="{urlmake sort="num" nsort=$sort.num}" class="item{if $cur_sort=='num'} {$cur_n}{/if}" rel="nofollow"><i>{t}наличию{/t}</i></a></li>
                    <li><a href="{urlmake sort="title" nsort=$sort.title}" class="item{if $cur_sort=='title'} {$cur_n}{/if}" rel="nofollow"><i>{t}названию{/t}</i></a></li>
                    {if $can_rank_sort}
                    <li><a href="{urlmake sort="rank" nsort=$sort.rank}" class="item{if $cur_sort=='rank'} {$cur_n}{/if}" rel="nofollow"><i>{t}релевантности{/t}</i></a></li>
                    {/if}
                </ul>
            </div>
        </div>
