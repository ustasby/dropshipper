...
class ImportPhotos extends \RS\Controller\Admin\Front
{
    ...
    function actionIndex()
    {
        if ($this->url->isPost()) {           
            ...
            if (!$this->api->hasError() && ($nextstep == 3 || $this->api->uploadFile($this->url->files('zipfile')))) {
                ...
                return $this->result
                        ->addSection('callCrudAdd', $this->router->getAdminUrl('ajaxProcess', $this->post_data + array('step' => $nextstep)))
                        ->setSuccess(true);
    ...
    function actionAjaxProcess()
    {
        ...
        if ($step < 4 && !$error) {
            $this->view->assign(array(
                'next_url' => $this->router->getAdminUrl('ajaxProcess', array(), null),
                'params' => json_encode($this->post_data + array('pos' => $pos, 'step' => $step)),
            ));
        }
