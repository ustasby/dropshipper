...
class WatchNow extends \RS\Controller\Admin\Widget
{
    ...
    function actionIndex()
    {
        ...
        foreach($list as $n => $event) {
            ...
            $user_href = ($user['id'] > 0) ? $this->router->getAdminUrl('edit', array('id' => $user['id']), 'users-ctrl') : false;
            ...
            $path_href = $this->router->getAdminUrl(false, array('dir' => $product['maindir']), 'catalog-ctrl');
            
            $eventTime = strtotime($event->getEventDate());
            $item = array(
                'id' => $product['id'],
                'editUrl' => $this->router->getAdminUrl('edit', array('id' => $product['id']), 'catalog-ctrl'),
                'eventDate' => date(\RS\Helper\Tools::dateExtend('%k, H:i', $eventTime), $eventTime),
                'user' => array('href' => $user_href, 'name' => $user_name),
                'path' => array('href' => $path_href, 'line' => $path_line),
                'product' => $product
            );
