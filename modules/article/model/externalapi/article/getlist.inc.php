...
class GetList extends \ExternalApi\Model\AbstractMethods\AbstractGetList
{
    ...
    */
    ...
    {
        return array('id', 'id desc', 'title', 'title desc', 'dateof', 'dateof desc');
    }
        ...
        if (!empty($list)){
            if (in_array('images', $this->method_params['sections'])){//Если нужно добавить секцию с картинками
                $dao->getElement()->getPropertyIterator()->append(array(
                    'preview_text' => new \RS\Orm\Type\Varchar(array(
                        'description' => t('Текст анонса'),
                        'appVisible' => true,
                    )),
                    'date' => new \RS\Orm\Type\Varchar(array(
                        'description' => t('Дата'),
                        'appVisible' => true,
                    )),
                    'photos' => new \RS\Orm\Type\ArrayList(array(
                        'description' => t('Фото статьи'),
                        'appVisible' => true,
                    )),
                    'attached_products_arr' => new \RS\Orm\Type\ArrayList(array(
