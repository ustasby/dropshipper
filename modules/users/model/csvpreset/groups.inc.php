...
class Groups extends \RS\Csv\Preset\AbstractPreset
{
    ...
    protected
        ...
        $link_preset_id,
        $many_link_ormobject,
        $loaded_groups = array(), //Группы пользователей
        $loaded_groups_by_name = array(), //Группы пользователей к ключом по name
        $title;
    ...
    */
    function getColumns()
    {        
        $colums_propety = array($this->id.'-usergroups' => array(
            'key'  => 'usergroups',
    ...
    */
    function getColumnsData($n)
    {
        ...
        if (isset($this->rows[$id])) {
            $groups = array(); //Имена групп в которых состоит пользователь
