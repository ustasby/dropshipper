        ...
        if ($app_data !== null) {
            if (isset($app_data['date_of_expire'])) {
                ...
                if ($remaining_days > 0 && $remaining_days < 15) {
                    return t('До завершения подписки на мобильное приложение для домена %domain осталось %days [plural:%days:день|дня|дней]. Нажмите, чтобы продлить подписку.', array(
                        'domain' => $domain,
                        'days' => $remaining_days
                    ));
                }
                ...
                if ($remaining_days > -15 && $remaining_days < 0) {
                    return t('Подписка на мобильное приложение для домена %domain истекла. Нажмите, чтобы продлить подписку.', array(
                        'domain' => $domain
                    ));
                }
            }
