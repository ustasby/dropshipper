...
class AddonArchiveApi extends \RS\Module\AbstractModel\BaseModel
{
    ...
    public function fetchModuleInfo($filename)
    {
        ...
        $module_info = self::parseModuleConfig($config_content) + array('name' => $mod_name);
    ...
    public function getAddonType($filename)
    {
        ...
        if($code !== true){
            return $this->addError(t('Не удалось открыть архив. Код ошибки: %0', array($code)));
        }

        $root_items_count = $this->getZipItemsCount($zip);

        if($root_items_count != 1)
            return $this->addError(t('Архив дополения должен содержать только одну папку в корне. Найдено %0', array($root_items_count)));
