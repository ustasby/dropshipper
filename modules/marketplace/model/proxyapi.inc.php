...
class ProxyApi extends BaseModel
{
    ...
    public function request($url, $post_data = false)
    {
        ...
        $headers_to_send = array(
            ...
            'X-ReadyScript-ProxyUrl: '.$host.\RS\Router\Manager::obj()->getAdminUrl(false, array(), 'marketplace-proxy'),
    ...
    public function getLicensesInfo()
    {
        ...
        foreach($licenses as $one){
            $license_arr = array('license' => $one['license'], 'data' => $one['data'], 'crypt_type' => $one['crypt_type']);
