...
class Get extends \ExternalApi\Model\AbstractMethods\AbstractGet
{
    ...
    */
    function process($token = null, $zone = '')
    {
        ...
        */
        $banner_zone = $api->getById($zone, 'alias');
        ...
        $banner->getPropertyIterator()->append(array(
                    'mobile_banner_type' => new \RS\Orm\Type\Varchar(array(
                        'description' => t('Тип баннера'),
                        'appVisible' => true
                    )),
                    'mobile_link' => new \RS\Orm\Type\Varchar(array(
                        'description' => t('Страницы для показа пользователю'),
                        'appVisible' => true
                    )),
                    'mobile_menu_id' => new \RS\Orm\Type\Integer(array(
                        'description' => t('Страницы для показа пользователю'),
                        'appVisible' => true
                    )),
                    'mobile_product_id' => new \RS\Orm\Type\Integer(array(
                        'description' => t('Товар для показа пользователю'),
                        'appVisible' => true
                    )),
                    'mobile_category_id' => new \RS\Orm\Type\Integer(array(
