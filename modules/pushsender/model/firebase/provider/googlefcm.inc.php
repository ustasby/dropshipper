...
class GoogleFCM extends AbstractProvider
{
    ...
    protected
        $last_error,
        $last_response,
        $targets = array(), //Массив из устройств для отправки 
        $api_url;
    ...
    */
    protected function fillNotification($platform, $data, $push)
    {
        if ($platform == 'ios'){
            ...
            $data['notification']['body']  = $push->getPushBody();  //aps ключ нужен для iOS  
        }else{
           $data['data']['title']   = $push->getPushTitle();
           $data['data']['body'] = $push->getPushBody();      
        }
    ...
    */
    ...
    {
        $this->writeLog(t('Начало отправки уведомления %0 через провайдер %1', array($push->getId(), get_class($this))));
        ...
        try {                                
            ...
            $this->writeLog(t('Отправка отклонена по причине: %0', array($this->last_error)));            
    ...
    */
    function sendQuery($server_key, $platform, $push, $data)    
    {
        ...
        $this->writeLog(t('Получен ответ от API: %0', array($result)));            
