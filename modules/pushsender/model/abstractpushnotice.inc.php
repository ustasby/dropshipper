...
abstract class AbstractPushNotice
{
    ...
    */
    public function getId()
    {
        ...
        return str_replace(array('\\', '-model-push'), array('-', ''), $class);
    }
    ...
    */
    ...
    public function send()
    {
        ...
        if (!($app instanceof App\InterfaceHasPush)) {
            throw new Exception(t('Приложение %0 должно имплементировать интерфейс PushSender\Model\App\InterfaceHasPush', array($this->getAppId())));
        }
        
        if (!$this->isInList($app->getPushNotices())) {
            throw new Exception(t('Уведомление %0 должно быть задекларировано в методе getPushNotices в приложении %1', array(get_class($this), get_class($app))));
        }
