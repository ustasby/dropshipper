...
{elseif $fld.type == 'text'}
    <textarea name="{$fld.fieldname}" {if $fld.maxlength>0}maxlength="{$fld.maxlength}"{/if} class="{if $has_error} has-error{/if}" placeholder="{$fld.title}">{$values[$fld.alias]}</textarea>
