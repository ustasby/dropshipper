...
<form method="POST" class="formStyle checkoutForm">
    <div class="workArea noTopPadd">
        ...
        <div class="coItems">
            ...
            <table class="themeTable noMobile">
                <tbody>
                    ...
                    {if $cartdata.payment_commission}
                        <tr>
                            <td>{if $cartdata.payment_commission.cost>0}{t}Комиссия{/t}{else}{t}Скидка{/t}{/if} {t}при оплате через{/t} "{$order->getPayment()->title}":</td>
            ...
            <br>
            <input type="checkbox" name="iagree" value="1" id="iagree"> <label for="iagree">{t alias="соглашение с условиями" url={$router->getUrl('shop-front-licenseagreement')}}Я согласен с <a href="%url" class="licAgreement inDialog">условиями предоставления услуг</a>{/t}</label>
            <script type="text/javascript">
                $(function() {
                    $('.formSave').click(function() {
                        if (!$('#iagree').prop('checked')) {
                            alert(lang.t('Подтвердите согласие с условиями предоставления услуг'));
                            return false;
                        }
                    });
                });
