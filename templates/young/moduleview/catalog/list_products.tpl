        ...
        <div class="viewOptions">
            ...
            <div class="lineListBlock sortBlock">
                <a class="lineTrigger rs-parent-switcher">{if $cur_sort=='sortn'}{t}умолчанию{/t}
                                                          ...
                                                          {else}{t}цене{/t}{/if}</a>
                <ul class="lineList">
                    <li><a href="{urlmake sort="sortn" nsort=$sort.sortn escape=true}" class="item{if $cur_sort=='sortn'} {$cur_n}{/if}" rel="nofollow"><i>{t}умолчанию{/t}</i></a></li>
                    <li><a href="{urlmake sort="cost" nsort=$sort.cost escape=true}" class="item{if $cur_sort=='cost'} {$cur_n}{/if}" rel="nofollow"><i>{t}цене{/t}</i></a></li>
                    <li><a href="{urlmake sort="rating" nsort=$sort.rating escape=true}" class="item{if $cur_sort=='rating'} {$cur_n}{/if}" rel="nofollow"><i>{t}популярности{/t}</i></a></li>
                    <li><a href="{urlmake sort="dateof" nsort=$sort.dateof escape=true}" class="item{if $cur_sort=='dateof'} {$cur_n}{/if}" rel="nofollow"><i>{t}дате{/t}</i></a></li>
                    <li><a href="{urlmake sort="num" nsort=$sort.num escape=true}" class="item{if $cur_sort=='num'} {$cur_n}{/if}" rel="nofollow"><i>{t}наличию{/t}</i></a></li>
                    <li><a href="{urlmake sort="title" nsort=$sort.title}" class="item{if $cur_sort=='title'} {$cur_n}{/if}" rel="nofollow"><i>{t}названию{/t}</i></a></li>
                    {if $can_rank_sort}
                    <li><a href="{urlmake sort="rank" nsort=$sort.rank escape=true}" class="item{if $cur_sort=='rank'} {$cur_n}{/if}" rel="nofollow"><i>{t}релевантности{/t}</i></a></li>
                    {/if}                                                    
                </ul>
            </div>
        </div>
