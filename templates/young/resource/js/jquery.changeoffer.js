...
(function( $ ){
    $.fn.changeOffer = function( method ) {
        var defaults = {
            ...
            mainPicture      : ".image.main", //Класс главных фото
...
$(function() {
    $('[name="offer"]').changeOffer({
        ...
        onShowOfferPhotos: function(offer, context, data) //Индивидуально для темы young
        {
            if (offer.closest('.multiComplectations').length==0){ //Если мы находимся в карточке товара
                ...
                $.colorbox.remove();
                $('.product .images .main[rel="bigphotos"]').colorbox({
                   rel:'bigphotos',
