<div class="topicView">
    {foreach $list as $item}
        {$user=$item->getUser()}
        {if $item@first}
            <p class="date">{$item.dateof|dateformat:t("%e %v %Y, в %H:%M")}</p>
            <h2>{$topic.title}</h2>
            <p class="quest">{$item.message}</p>
        {else}
            <div class="message">
                <div class="author">
                    <span class="user">{if $item.is_admin}{t name=$user.name surname=$user.surname}%name %surname, администратор{/t}{else}{t}Я{/t}{/if}</span>
