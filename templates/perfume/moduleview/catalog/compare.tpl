        ...
        <table class="compareHead">
            <tr>
                ...
                <td class="compareItem{if !$product->isAvailable()} notAvaliable{/if}" data-id="{$product.id}">
                    <a title="{t}Исключить из сравнения{/t}"  class="remove"><span>{t}удалить{/t}</span></a>
                    <div class="image{if count($imglist)>1} activelist{/if}">
                        {foreach $imglist as $image}
                            <img src="{$image->getUrl(226,236)}" {if !$image@first}class="hidden"{/if} alt="{$image.title}"/>
                        {/foreach}
                        {if !count($imglist)}
                    ...
                    <div class="actions">
                        ...
                        {if $shop_config}
                        <a data-href="{$router->getUrl('shop-front-cartpage', ["add" => $product.id])}" data-add-text="Добавлено" class="addToCart noShowCart">В корзину</a>
                        {/if}
...
<script>
    $(function() {
        $(window).scroll(function(){
          $('.compareHeadWrap').css('top',$(window).scrollTop());
        });    
    });
</script>
{else}
