...
if (\RS\AccessControl\Rights::CheckRightError('main', ACCESS_BIT_WRITE) !== false) {
    response(trans('Access denied!'), 403)->send();
    exit;
}
...
$path = $base_path.$current_path.$_POST['path'];
$path_thumb = $base_path.$_POST['path_thumb'];
if (isset($_POST['name']))
{
...
if (isset($_GET['action']))
{
    switch($_GET['action'])
    {
            ...
            if ($delete_files){
                ...
                if ($fixed_image_creation)
                {
                    foreach($fixed_path_from_filemanager as $k=>$path)
                    {
                        ...
                        $base_dir=$path.substr_replace($info['dirname']."/", '', 0, strlen($base_path.$current_path));
                ...
                if (is_dir($path))
                {
                    ...
                    if ($fixed_image_creation)
                    {
                        foreach($fixed_path_from_filemanager as $k=>$paths){
                            ...
                            $base_dir=$paths.substr_replace($path, '', 0, strlen($base_path.$current_path));
                ...
                if (!empty($name))
                {
                    ...
                    if ($fixed_image_creation)
                    {
                        ...
                        foreach($fixed_path_from_filemanager as $k=>$paths)
                        {
                            ...
                            $base_dir = $paths.substr_replace($info['dirname']."/", '', 0, strlen($base_path.$current_path));
                            if (file_exists($base_dir.$fixed_image_creation_name_to_prepend[$k].$info['filename'].$fixed_image_creation_to_append[$k].".".$info['extension']))
                            {
        ...
        case 'paste_clipboard':
            ...
            $data['path'] = $base_path.$current_path.$data['path'];
            $data['path_thumb'] = $base_path.$data['path_thumb'];
