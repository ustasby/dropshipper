...
if (file_exists($base_path.'/setup.inc.php')) {
    require($base_path.'/setup.inc.php');
} else {
    $base_path = realpath(__DIR__.'/../../../../../');
    if (file_exists($base_path.'/setup.inc.php')) {
        require($base_path.'/setup.inc.php');
    } else {
        die('Setup not found at: '.$base_path.'/setup.inc.php');
    }
}                                   

//Если была установка в подпаку
if (!empty(\Setup::$FOLDER) && (mb_stripos($base_path, \Setup::$FOLDER, 0, 'UTF-8')!== false)){
    $base_path = rtrim($base_path, \Setup::$FOLDER);
}
...
{
    echo t('Только администраторы имеют доступ к загрузке файлов. Авторизуйтесь в административной панели с флагом <b>Запомнить меня</b>.');
    exit;
}
...
$config = array(
	...
	*/
	...
	|
	*/
	'upload_dir' => \Setup::$FOLDER.'/storage/tiny/',
	...
	|
	*/
	'current_path' => \Setup::$FOLDER.'/storage/tiny/',
	...
	|
	*/
	'thumbs_base_path' => \Setup::$FOLDER.'/storage/tiny_thumbs/',
	...
	*/
	...
	'access_keys' => array(sha1(\Setup::$SECRET_KEY.\Setup::$SECRET_SALT)),
...
\RS\File\Tools::makePath(\Setup::$ROOT.$config['thumbs_base_path']);
\RS\File\Tools::makePath(\Setup::$ROOT.$config['upload_dir']);
