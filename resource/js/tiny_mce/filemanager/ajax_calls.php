...
if(isset($_GET['action']))
{
    switch($_GET['action'])
    {
		...
		case 'image_size': // not used
			...
			if ($pos !== false)
			{
				$info = getimagesize(substr_replace($_POST['path'], $base_path.$current_path, $pos, strlen($upload_dir)));
				response($info)->send();
				exit;
			}
			break;
		case 'save_img':
			...
			$fp = fopen($base_path.$current_path . $_POST['path'] . $_POST['name'], "w");
			fwrite($fp, $image_data);
			fclose($fp);

		    create_img($base_path.$current_path.$_POST['path'].$_POST['name'], $base_path.$thumbs_base_path.$_POST['path'].$_POST['name'], 122, 91);
		    // TODO something with this function cause its blowing my mind
		    new_thumbnails_creation(
				$base_path.$current_path.$_POST['path'],
				$base_path.$current_path.$_POST['path'].$_POST['name'],
				$_POST['name'],
		...
		case 'extract':
			...
			$path = $base_path.$current_path . $_POST['path'];
			$info = pathinfo($path);
			$base_folder = $base_path.$current_path . fix_dirname($_POST['path']) . "/";
			...
			switch ($info['extension'])
			{
				...
				case "tar":
					...
					$phar->extractTo($base_path.$current_path . fix_dirname($_POST['path']) . "/", $files, true);
		...
		case 'get_file': // preview or edit
			...
			$selected_file = ($sub_action == 'preview' ? $_GET['file'] : $base_path.$current_path . $_POST['path']);
